...
import '../models/trip_plan.dart';  // 添加這行導入語句
                ...
                child: ElevatedButton(
                  onPressed: () {
                    if (_formKey.currentState!.validate() && 
                      ...
                      // 驗證數據是否正確
                      print('檢查行程資料：');
                      print('行程名稱: ${tripPlan.name}');
                      print('開始時間: ${_formatDateTime(tripPlan.startTime)}');
                      print('結束時間: ${_formatDateTime(tripPlan.endTime)}');
                      print('景點數量: ${tripPlan.attractions.length}');

                      // 顯示成功訊息
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text('行程 "${tripPlan.name}" 已成功建立'),
                          duration: const Duration(seconds: 2),
                        ),
                      );

                      // 返回新創建的行程規劃
                      Navigator.pop(context, tripPlan);
                    } else {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
